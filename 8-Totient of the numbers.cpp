...
int main()
{
    ...
    do
    {
        ...
        scanf(" %c", &ch); // The space before %c is used to consume any whitespace characters
        if (ch == 'n' || ch == 'N')
        {
    ...
    } while (ch == 'y' || ch == 'Y'); // Loop continues only if 'y' or 'Y' is entered
...
void computeTotient(int n)
{
    ...
    long long *phi = (long long *)malloc((n + 1) * sizeof(long long));
    for (int i = 1; i <= n; i++)
